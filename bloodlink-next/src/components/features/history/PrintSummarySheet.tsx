...
export const PrintSummarySheet = ({ patients, signature, hospitalName = 'โรงพยาบาลส่งเสริมสุขภาพตำบล', isPdfMode = false }: PrintSummarySheetProps) => {
    ...
    return (
        <div className={isPdfMode ? "bg-white text-black font-[family-name:var(--font-kanit)]" : "w-full text-black font-[family-name:var(--font-kanit)]"}>
            ...
            {chunkedPatients.map((pagePatients, pageIndex) => (
                <div key={pageIndex} className={`summary-sheet-container relative bg-white mx-auto ${isPdfMode ? 'w-[1123px] min-h-[794px] p-8' : 'w-full max-w-[297mm]'} ${pageIndex < chunkedPatients.length - 1 && !isPdfMode ? 'page-break' : ''}`}>
                    {/* Header */}
                    ...
                    <div className="flex justify-between mt-4 text-[12px] border-t border-dashed border-gray-400 pt-3 relative">
                        ...
                        <div className="flex flex-col gap-1 items-end w-1/3 border-l border-dashed border-gray-300 pl-2">
                            <span className="font-bold">หน่วยงานปลายทาง (ห้องแล็บ รพช.):</span>
                            <div className="flex gap-4 my-2">
                                <label className="flex items-center gap-1"><div className="w-3 h-3 border border-gray-400"></div> สิ่งส่งตรวจถูกต้อง</label>
                                <label className="flex items-center gap-1"><div className="w-3 h-3 border border-gray-400"></div> ไม่ถูกต้อง</label>
                            </div>
                            <div className="flex flex-col items-center gap-1 mt-1 w-full max-w-[200px] ml-auto">
                                <span className="text-gray-500 font-bold border-b border-gray-400 border-dotted w-full h-[18px]"></span>
                                <span className="text-gray-500 text-[10px]">(ลงชื่อผู้รับเรื่อง/ตรวจวิเคราะห์)</span>
                            </div>
                        </div>
                    </div>
                </div>
