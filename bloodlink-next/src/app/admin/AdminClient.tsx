...
import { Loader2, Database, ShieldCheck, Settings, Clock } from 'lucide-react';
...
export function AdminClient({ initialStats, initialSystemInfo }: AdminClientProps) {
    ...
    // Initial check for DB Status to mirror what was on the main dashboard
    useEffect(() => {
        if (initialSystemInfo.dbStatus === 'checking') {
            const checkDb = async () => {
                try {
                    const res = await fetch('/api/health'); // Assuming there's a basic health or we just check if stats loaded
                    if (res.ok || initialStats !== null) {
                        setSystemInfo(prev => ({ ...prev, dbStatus: 'connected' }));
                    } else {
    ...
    }, [initialStats, initialSystemInfo.dbStatus]);
    ...
    return (
        <div className="flex flex-col gap-6 pb-6">
            <h1 className="text-[24px] font-bold text-[#111827] dark:text-white animate-fade-in-up">แดชบอร์ดผู้ดูแลระบบ</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-5 min-h-[300px] lg:min-h-[420px]">
                ...
                <div className="bg-white dark:bg-[#1F2937] rounded-[16px] p-6 shadow-[0_2px_4px_rgba(0,0,0,0.05)] dark:shadow-none flex flex-col relative transition-colors border border-transparent dark:border-gray-700 animate-fade-in-up stagger-6">
                    ...
                    {/* Legend */}
                    <div className="flex justify-center flex-wrap gap-4 mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                        <div className="flex items-center gap-2">
            ...
            {/* System Info Panel (Moved from Main Dashboard) */}
            <div className="bg-white dark:bg-[#1F2937] rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 transition-colors card-animate stagger-8 mt-1">
