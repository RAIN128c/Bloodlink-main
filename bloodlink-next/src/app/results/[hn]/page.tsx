            ...
            case 'Normal':
                return (
                    <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800">
                        ...
                        <span className="text-sm font-semibold text-emerald-700 dark:text-emerald-400">ผลปกติ (Normal)</span>
                    </div>
                );
            case 'Abnormal':
                return (
                    <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800">
                        <AlertTriangle className="w-5 h-5 text-amber-500" />
                        <span className="text-sm font-semibold text-amber-700 dark:text-amber-400">ผิดปกติ (Abnormal)</span>
                    </div>
                );
            case 'Critical':
                return (
                    <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800">
                        <AlertCircle className="w-5 h-5 text-red-500" />
                        <span className="text-sm font-semibold text-red-700 dark:text-red-400">วิกฤต (Critical)</span>
                    </div>
                );
            default:
                return (
                    <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                        ...
                        <span className="text-sm text-gray-500 dark:text-gray-400">ยังไม่มีผลสรุป</span>
                    </div>
                );
        }
    };

    const getSmallSummaryBadge = (summary?: string) => {
        switch (summary) {
            case 'Normal':
                return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400"><CheckCircle className="w-3 h-3" /> ปกติ</span>;
            case 'Abnormal':
                return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400"><AlertTriangle className="w-3 h-3" /> ผิดปกติ</span>;
            case 'Critical':
                return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400"><AlertCircle className="w-3 h-3" /> วิกฤต</span>;
            default:
        ...
        <div className="flex min-h-screen bg-[#F3F4F6] dark:bg-[#0f1729] font-[family-name:var(--font-kanit)]">
            ...
            <main className="flex-1 md:ml-[195px]">
                ...
                <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
                    {/* Back Link */}
                    <Link href="/results" className="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 mb-4 transition">
                        ...
                        <>
                            ...
                            <div className="bg-white dark:bg-[#1F2937] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-6 mb-6">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                    ...
                                    {getSummaryBadge(latestResult?.result_summary)}
                                </div>
                            </div>
                                                ...
                                                {history.map((result, idx) => (
                                                    ...
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            {getSmallSummaryBadge(result.result_summary)}
                                                        </div>
